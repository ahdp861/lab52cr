...
namespace RetailDatabaseManager
{
    ...
    {
        private Dictionary<string, List<Dictionary<string, string>>> _database;
        public Logger Logger { get; }

        public DatabaseManager()
        {
            _database = new Dictionary<string, List<Dictionary<string, string>>>();
        ...
        {
            if (!_database.ContainsKey(sheetName))
            {
        ...
        private void AddRecordMenu()
        {
            ...
            try
            {
                ...
                var sampleRecord = _database[sheetName].FirstOrDefault();
                ...
                _database[sheetName].Add(newRecord);
        ...
        private void EditRecordMenu()
        {
            ...
            try
            {
                ...
                var records = _database[sheetName];
        ...
        private void QueryTopSellingProducts()
        {
            try
            {
                if (!_database.ContainsKey("Движение товаров") || !_database.ContainsKey("Товар"))
                {
                    Console.WriteLine("Ошибка: Необходимые листы не найдены в базе данных.");
                    return;
                }

                var sales = _database["Движение товаров"];
                var products = _database["Товар"];
        ...
        private void QueryMostDiscountedProducts()
        {
            try
            {
                if (!_database.ContainsKey("Товар"))
                {
        ...
        private void QuerySalesByStores()
        {
            try
            {
                if (!_database.ContainsKey("Движение товаров") || !_database.ContainsKey("Магазин"))
                {
                    Console.WriteLine("Ошибка: Необходимые листы не найдены в базе данных.");
                    return;
                }
        ...
        private void QueryProductsWithoutSales()
        {
            try
            {
                if (!_database.ContainsKey("Товар") || !_database.ContainsKey("Движение товаров"))
                {
                    Console.WriteLine("Ошибка: Необходимые листы не найдены в базе данных.");
                    return;
                }
